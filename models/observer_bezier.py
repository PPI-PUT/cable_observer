...
class Cable(tf.keras.Model):
    ...
    def __init__(self, N, xy=None):
        super(Cable, self).__init__()
        self.N = N
        # initialize with horizontally oriented straight cable
        if xy is None:
            s = np.linspace(0.1, 0.9, self.N + 1)
            xy = np.stack([0.5 * np.ones(self.N + 1), s], axis=-1)
