...
from models.bezier_supervised import _plot, CableNetwork, Encoder, Decoder
...
def main(args):
    ...
    # 2. Define model
    encoder = Encoder()
    encoder.load_weights("pretrained/encoder_best_20214")
    decoder = Decoder()
    decoder.load_weights("pretrained/decoder_best_20214")

    # 3. Optimization

    optimizer_encoder = tf.keras.optimizers.Adam(args.eta)
    optimizer_decoder = tf.keras.optimizers.Adam(args.eta)
    l2_reg = tf.keras.regularizers.l2(1e-5)

    # 4. Restore, Log & Save
    experiment_handler = ExperimentHandler(args.working_path, args.out_name, args.log_interval, encoder, optimizer_encoder)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, data in _ds('Train', dataset_epoch, train_size, epoch, args.batch_size):
            ...
            with tf.GradientTape(persistent=True) as tape:
                gt_cps, img = data
                lv = encoder(img)
                output_img = decoder(lv)
                #model_loss = tf.losses.binary_crossentropy(img, output_img, from_logits=True)
                b = tf.reduce_sum(img[..., 0], axis=(-2, -1))
                c = tf.reduce_sum(img[..., 1], axis=(-2, -1))
                #w = tf.math.log(b) - tf.math.log(c)
                w = 1.0
                model_loss = tf.nn.weighted_cross_entropy_with_logits(img, output_img, w)
                model_loss = tf.reduce_sum(model_loss, axis=(-3, -2, -1))
            # 5.1.2 Take gradients (if necessary apply regularization like clipping),
            grads_encoder = tape.gradient(model_loss, encoder.trainable_variables)
            optimizer_encoder.apply_gradients(zip(grads_encoder, encoder.trainable_variables))
            grads_decoder = tape.gradient(model_loss, decoder.trainable_variables)
            optimizer_decoder.apply_gradients(zip(grads_decoder, decoder.trainable_variables))

            preds = tf.nn.softmax(output_img, axis=-1)
            ...
            with tf.summary.record_if(train_step % args.log_interval == 0):
                ...
                tf.summary.image('img/output', output_img[..., 1:], step=train_step)

            # 5.1.5 Update meta variables
            train_step += 1
        epoch_accuracy = tf.reduce_mean(tf.concat(acc, 0))
        epoch_loss = tf.reduce_mean(tf.concat(loss, 0))
        ...
        if epoch_loss < best_loss:
            ...
            encoder.save_weights("encoder_best_" + str(epoch))
            decoder.save_weights("decoder_best_" + str(epoch))
            best_list.append(epoch)
            if len(best_list) > 2:
                nr = best_list.pop(0)
                for f in glob("*_best_" + str(nr) + ".*"):
                    os.remove(f)
